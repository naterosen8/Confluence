...
Everything except the bars is *derived*, which means a change to how a number is computed does not reach the site until the next scheduled run — a commit that adds a column would ship a site whose rows do not have it. `npm run rebuild` recomputes all four derived files — `screener.json`, `correlations.json`, `ticker-record.json` and `track-record-summary.json` — from the committed bars and log with no network access, no API key and no rate limit, and never writes a bar file, so it cannot invent or alter a single price. It carries the existing `generatedAt` forward rather than restamping, so the staleness check keeps telling the truth.
...
There will usually be a handful, and the module refuses to turn a handful into a percentage. Below **25** resolved calls no rate is quoted at all — not "quoted with a caveat", not quoted, because a percentage printed beside a sample of three is read as a percentage and the explanation underneath in smaller type is not a fair fight. The read says so plainly instead: *"100% of 3 and 0% of 3 are the same evidence about this ticker, which is none"*, with a figure for how many calls it would actually take. Above the threshold, the rate is always shown against that ticker's own drift with the interval on the difference, the same null everything else here tests against.

The threshold was **10 and is now 25**, and how it moved is the point. Ten was chosen before any ticker had reached it — a guess about what ten would look like rather than an observation, written in a comment that called ten "still far too few to conclude anything", which should have been the tell. When the record grew and 52 tickers crossed it at once, what ten actually produced was SLB reading *"100% of 10"* against a drift of 100%, T and VZ at 90%, TSLA at 7%, and a median interval width across all 52 of **fifty percentage points**. A page reading "100%" is not rescued by the bracket beside it. Twenty-five is not enough to conclude anything either; it is the point at which a number is worth printing at all.
...
**The N is not the N, and this is now corrected rather than caveated.** That paragraph used to end "stated rather than corrected for, because the correction needs an assumption less defensible than the caveat." That was wrong, and the size of the error only became visible once the record grew: 260 resolved calls were made on **ten distinct days**, seventy-four of them on one date, and at a five-session hold those ten dates contain about **two non-overlapping episodes**. Every call made on one session is scored over the same five sessions of the same market — those are not seventy-four tests of anything, they are one market week wearing seventy-four tickers.

The fix needs no shaky assumption because it gives up power rather than borrowing it: group calls by the session they fired on, score each day once, and put the interval on the spread across days (`lib/clustering.js`). A day with two calls then counts the same as a day with seventy-four, which is the right trade when the alternative is a confidence claim the data cannot support. The published range moved from 43.6–55.7% to **36.8–64.2%** — 2.3 times wider — and the wider one is now the headline, with the narrow one shown beside it and the reason it is wrong. The directional gaps carry the same dependence and are optimistic by roughly the same factor, which the page says.

This is the third correction of this kind, and they all run the same direction: every error of this sort flatters the site, so nothing catches them except looking.
...
**Survivable size.** A position opened at every entry in the window, at each rung the simulator offers, walked forward. The largest rung that lost none of them.

This figure is a **minimum over the window**, which makes it an extreme-value statistic with an easy-to-miss consequence: it can only ever fall as the window grows, because a longer window has had more chances to find the entry that would have killed the position. The site was publishing a 250-entry window. Over 500, **39 of 89 tickers read lower and none read higher** — HYG 50× to 10×, TLT 25× to 10×, SPY 10× to 5×, and five crypto pairs from 3× to **1×**, meaning sizes the shorter window called survivable had in fact been liquidated, just not inside the part of history being looked at.

More history is strictly better for a floor, so the floor is now taken over as much of it as there is, and the 250-entry answer is reported beside it. Anyone who thinks a regime change makes older history the wrong evidence has a real argument — but it is theirs to make, not the site's to make silently by picking a short window. Where the two disagree the read says so outright: *"Recent history alone would say 10x… the lower figure is the one stated, because this is a floor — the shorter window is not a second opinion, it is the same measurement with less of the evidence."* Reported as an instruction because it is a fact about a distribution rather than a guess about the next draw — and stated plainly as *not* a recommendation to use that size, not a level anyone has checked is safe, but the point past which the instrument has already gone through a position. The caveat also now states how many of the entries are genuinely independent: at a five-session hold, 500 consecutive entries are about 100 non-overlapping episodes.
...
**It is survivorship-biased anyway, and the methodology page used to claim it was not.** It said a fixed curated set "avoids survivorship bias in the sense that nothing is removed for performing badly." That is backwards: the selection already happened, at the end of the sample. These names were chosen because they are recognisable *today*, so every one survived to be chosen; the list this site would have assembled in 2022 contained names that are not here, missing precisely because of what happened to them. Nothing being removed going forward does not undo a list picked at the finish line. Every base rate and every drift baseline computed from these bars is conditional on the instrument having survived, and a free data tier does not carry delisted names, so it is stated rather than fixed.

A **"five-session hold" is also not the same span of time across the board**: crypto trades every day, so five sessions is five calendar days, while an equity's five sessions span seven and eleven across a long weekend. Every cross-instrument comparison here compares holds of different real duration.
...
Every derived file the site publishes now has a guard that **recomputes it from its source and compares**: `screener.json` and `correlations.json` from the committed bars, `ticker-record.json` and `track-record-summary.json` from the committed log. That closes the gap where a change to how a figure is computed reaches the code and not the file, and it is checked against what the production job actually wrote — the offline `npm run rebuild` and the daily sync produce byte-identical output.
